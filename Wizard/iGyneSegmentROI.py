...
class iGyneAnalyzeROIStep( iGyneStep ) :
  ...
  def __init__( self, stepid ):
    ...
    self.__parent = super( iGyneAnalyzeROIStep, self )
...
#    self.buttonBoxHints = self.ButtonBoxHidden
    ...
    # metrics are named as iGyne*Metric
    allModules = dir(slicer.moduleNames)
    iGyneMetrics = []
    for m in allModules:
      if m.endswith('Metric'):
        iGyneMetrics.append(m)

    print 'Metrics discovered: ', iGyneMetrics

    # if len(iGyneMetrics) == 0:
  ...
  def onEntry(self, comingFrom, transitionType):
    ...
    super(iGyneAnalyzeROIStep, self).onEntry(comingFrom, transitionType)
  ...
  def onExit(self, goingTo, transitionType):
    ...
    super(iGyneAnalyzeROIStep, self).onExit(goingTo, transitionType)
