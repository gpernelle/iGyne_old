...
#    self.__layout.addRow( followupScanLabel, self.__followupVolumeSelector )
    ...
    checkButton1.connect('clicked()',self.procedureSelector)
    radioButton2.connect('clicked()',self.procedureSelector)
    radioButton3.connect('clicked()',self.procedureSelector)
    radioButton4.connect('clicked()',self.procedureSelector)
    radioButton5.connect('clicked()',self.procedureSelector)

    self.updateWidgetFromParameters(self.parameterNode())

    qt.QTimer.singleShot(0, self.killButton)
  
  def procedureSelector(self):
...
#      self.__parent.validationFailed(desiredBranchId, 'Error','Please select both baseline and followup volumes!')
  ...
  def validate(self, desiredBranchId):
    procedure = self.__procedureSelector
...
#    if goingTo.id() != 'SelectApplicator':
#      return
